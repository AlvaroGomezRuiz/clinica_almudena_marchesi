...
from utils.security import decrypt_data, encrypt_data, get_current_user
from utils.config import settings
from db.session import get_db
from models.base import Cita, HistorialSesiones, Paciente, Servicio, Usuario
...
def crear_paciente(
    ...
    db_paciente = (
        db.query(Paciente)
        .filter(
            (Paciente.dni_nie_bidx == dni_bidx) | (Paciente.email == paciente.email)
        )
        .first()
    )
    ...
    motivo_consulta_encriptado = (
        encrypt_data(paciente.motivo_consulta) if paciente.motivo_consulta else ""
    )
...
@router.get("/{paciente_id}/historial", response_model=PacienteHistorialResponse)
def obtener_historial_paciente(
    ...
    rows = (
        db.query(HistorialSesiones, Cita, Servicio)
        .join(Cita, HistorialSesiones.cita_id == Cita.id)
        .join(Servicio, Cita.servicio_id == Servicio.id)
        .filter(
            HistorialSesiones.activo == True,
            HistorialSesiones.paciente_id == paciente_id,
        )
        .order_by(Cita.inicio_iso.asc())
        .all()
    )

    items_asc: list[PacienteSesionItem] = []
    for idx, (sesion, cita, servicio) in enumerate(rows, start=1):
        notas_raw = getattr(sesion, "notas_clinicas", None)
        tareas_raw = getattr(sesion, "tareas_asignadas", None)
        estado_emocional_raw = getattr(sesion, "estado_emocional", None)

        notas = _decrypt_if_fernet(notas_raw) or ""
        tareas = _decrypt_if_fernet(tareas_raw) or ""
        estado_emocional = _decrypt_if_fernet(estado_emocional_raw) or ""

        fecha_registro = getattr(sesion, "fecha_registro", None) or getattr(
            cita, "inicio_iso"
        )
        ...
        items_asc.append(
            PacienteSesionItem(
                ...
                servicio_nombre=getattr(servicio, "nombre", "") or "",
                estado_cita=getattr(cita, "estado", "") or "",
    ...
    return PacienteHistorialResponse(total=len(items_desc), items=items_desc)
