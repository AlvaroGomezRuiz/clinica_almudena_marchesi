...
function parseBirthDate(value: string): Date | null {
  ...
  const [ddRaw, mmRaw, yyyyRaw] = value.split('/');
...
type ExperienceValue = 'primera_vez' | 'hace_tiempo' | 'recientemente' | '';

export default function RegistroPacienteClient(): JSX.Element {
  ...
  const handleBirthDateTextChange = (e: ChangeEvent<HTMLInputElement>) => {
    const val = formatBirthDate(e.target.value).slice(0, 10);
    setBirthDateText(val);
    const parsed = parseBirthDate(val);
    setBirthDate(parsed ?? undefined);
  };

  const handleSelectDate = (newDate: Date | undefined) => {
    setBirthDate(newDate);
    if (newDate) {
      setBirthDateText(format(newDate, 'dd/MM/yyyy'));
    }
  };
          ...
          <ScrollReveal className="mb-14">
            <div className="max-w-2xl">
              ...
              <p className="font-display text-[1.2rem] text-ink-soft leading-relaxed text-pretty">
                Tu bienestar comienza en un entorno de confianza. Toda la información está protegida bajo los
                más estrictos estándares de confidencialidad clínica.
              </p>
            </div>
          </ScrollReveal>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden transition-colors duration-500">
                  ...
                  <div className="relative z-10">
                    ...
                    <blockquote className="border-l-2 border-sage/30 pl-5 mb-8">
                      <p className="font-display text-lg text-ink italic leading-relaxed">
                        &ldquo;Este espacio ha sido diseñado para que te sientas seguro compartiendo tu
                        historia.&rdquo;
                      </p>
                      <div className="mt-6">
                        <h2 className="font-display text-lg text-ink">¿Ya tienes una cuenta?</h2>
                        <a href="/login" className="text-sage font-medium hover:underline transition-all">
                          INICIAR SESIÓN
                        </a>
                      </div>
                    </blockquote>
                ...
                <div className="lg:col-span-8 p-10 lg:p-12 bg-transparent">
                  ...
                  <form action={startRegistrationAction} className="space-y-10">
                    ...
                    <input type="hidden" name="client_timestamp" value={new Date().toISOString()} />
                    ...
                    <div>
                      <label className={labelClasses}>¿Has estado en terapia antes?</label>
                        ...
                        {[
                          ...
                          <label key={opt.value} className="group cursor-pointer">
                            <input
                              ...
                              onChange={() => setExperience(opt.value as ExperienceValue)}
                            />
                            <PremiumCard tilt={false} active={experience === opt.value}>
                    ...
                    <div>
                      <label className={labelClasses}>Cuéntame, ¿qué te trae por aquí?</label>
                      <textarea
                    ...
                    <div className="pt-6 border-t border-line">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <span className="font-display text-[1.1rem] text-ink-soft leading-relaxed">
                          ...
                          <strong>RGPD (UE) 2016/679</strong> y la <strong>Ley 41/2002</strong> de
                          Autonomía del Paciente. Autorizo a Almudena Marchesi Fernández (Col.
                          M-40804) al tratamiento de mis datos clínicos con la única finalidad de
                          prestar el servicio terapéutico solicitado.
                        </span>
                      </label>
                    </div>
