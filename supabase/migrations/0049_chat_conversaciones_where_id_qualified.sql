-- 0049 — calificar `id` en chat_enviar_mensaje (ambiguity con OUT "id" de RETURNS TABLE)
-- -----------------------------------------------------------------------------
-- 0048 evitó RETURNING *; el 42702 puede seguir en:
--   where id = p_conversacion_id
-- porque `id` también es el nombre de la columna de salida (variable OUT)
-- y Postgres lo ve ambiguo frente a conversaciones.id.
-- Solución: alias de tabla (c.id) en SELECT y UPDATE de conversaciones.
