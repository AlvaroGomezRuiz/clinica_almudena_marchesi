-- 0057 — bono_asignar_manual: métodos tarjeta | transferencia | regalo | klarna
-- Elimina 'otro'. Sustituye el set anterior (efectivo/otro) por etiquetas alineadas con la UI admin.
...
begin
  ...
  if p_metodo not in ('tarjeta', 'transferencia', 'regalo', 'klarna') then
...
comment on function public.bono_asignar_manual(uuid, uuid, int, text, int, int, text, boolean) is
  'Asignación manual de bono + pago. p_metodo: tarjeta | transferencia | regalo | klarna.';
