...
> **Alcance**: Supabase Edge Functions (Deno) + Postgres/RPC.
> **Fecha auditoria original**: 2026-04-21. **Actualizacion 2026-04-23**: la carpeta `backend/` (FastAPI) **ya no existe** en el repositorio (hito 14); las secciones que la citan quedan como **referencia historica**.

---

## 1. Resumen ejecutivo

El backend productivo vive **integramente en Supabase** (Postgres + Edge Functions + Realtime). ~~FastAPI en `backend/`~~ fue retirado del monorepo; nueva logica servidor → RPC o Edge Function.

| Componente              | LOC aprox. | Estado     | Cobertura tests |
|-------------------------|------------|------------|-----------------|
| Edge Functions (10+)    | ~2.800+    | produccion | smoke test manual |
| FastAPI (legacy)        | —          | **eliminado del repo** | — |
| Migraciones SQL         | ~3.500+    | produccion (0001..0046 + stubs historial) | n/a |
| RPCs (~40+)             | -          | produccion | roundtrip manual |

**Veredicto**: codigo productivo (Edge + SQL) solido y defensivo. El informe histórico sobre FastAPI debajo sirve solo si se recuperara código desde git antiguo.
...
| Resuelta  | ~~Dual stack DB (MySQL vs Postgres)~~     | ~~Decidir cual es fuente de verdad~~ → **MySQL retirado el 22-abr-2026** (ver `docs/00_proyecto/cronologia.md`, Hito 12). FastAPI retirado del repo (Hito 14). |
