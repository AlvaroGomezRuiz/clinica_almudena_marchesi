...
│  • App Router + RSC          │              │  • Postgres 17 + RLS            │
...
| Capa              | Tecnologia                         |
|-------------------|-------------------------------------|
| Base de datos     | Postgres 17 (proyecto enlazado; `supabase/config.toml` `[db] major_version = 17`) |
...
### Backend legacy
**FastAPI / carpeta `backend/` retirados del repo** (hito 14). La única API de negocio es PostgREST + RPCs + Edge Functions.
...
│
├─ supabase/
│  ├─ migrations/                  # 0001..0046 + stubs 202604*_remote_reconcile (historial CLI)
