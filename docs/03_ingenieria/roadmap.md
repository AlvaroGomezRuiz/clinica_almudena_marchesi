...
- [x] **Cancelacion cita paciente >48h** — migración `0046` (sin refund Stripe automático al paciente; admin conserva refund cuando procede). Pendiente: reembolso parcial *UX/policy* si se define regla distinta.
...
### Observabilidad
- [x] **Runbook alertas operativas** documentado en `docs/05_operaciones/alertas-operativas.md` (7 reglas Sentry + pago fallido + Resend + uptime).
...
Registro de lo entregado en la ultima pasada senior (detalle en `docs/00_proyecto/estado-y-pendientes.md` §7):
