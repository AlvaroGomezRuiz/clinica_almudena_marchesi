...
Última actualización: **2026-04-27** — **Refinamientos de la app** (segunda pasada: auditoría en Configuración con nombre de paciente coherente, adjuntos en ficha con subida/descarga admin, cancelación de cita desde panel de agenda, refinamientos de panel mensajes/listados/pagos/web pública): **`docs/00_proyecto/refinamientos-app-2026-04-27.md`**. **Mismo día (mismo doc):** avatar en menú usuario admin/portal, estilo de **huecos ocupados** en reserva online, icono **Información** en ficha rápida del chat admin (`ContactFichaIcon`). **Migraciones:** la RPC `terapeuta_public_profile` está en **`0065_terapeuta_public_profile.sql`** (antes duplicaba el número **`0062`** con `0062_cuadricula_reserva_slots.sql`, lo que rompía `supabase db push` con clave duplicada en `schema_migrations`). La pasada **26-abr** sigue en **`refinamientos-app-2026-04-26.md`** (migración **`0064`**: aplicar en remoto si aún no está; revisar staging antes del `DELETE` fuera de franja).
...
| Dato | Estado | Uso |
|------|--------|-----|
| **Dominio** | **Producción canónica en código:** `ampsicologia.es` (abr-2026). Nombre reservado / checklist histórico: **amclinicapsicologia.es** — **reconciliar** (un solo canónico + redirects) antes de cierre DNS final. | Vercel + Resend + Supabase Auth redirects + `NEXT_PUBLIC_*_URL` + `FRONTEND_URL`. |
...
| **REGCESS** | *No aplica / nulo* (no rellenar secret) | `FACTURA_EMISOR_REGCESS` vacío: el PDF no imprime la línea. |
...
Valores de referencia para **Supabase → Project Settings → Edge Functions → Secrets** (replicar en el panel; el PDF concatena `DIRECCION` + salto de línea + `CP_CIUDAD`):

| Secret | Valor |
|--------|--------|
| `FACTURA_EMISOR_NOMBRE` | `ALMUDENA MARCHESI FERNANDEZ` |
| `FACTURA_EMISOR_NIF` | `04850571D` |
| `FACTURA_EMISOR_DIRECCION` | `Calle de Meléndez Valdés Nº 22, 1ºD` |
| `FACTURA_EMISOR_CP_CIUDAD` | `28015 MADRID` |
| `FACTURA_EMISOR_EMAIL` | `clinica.almudena.marchesi@outlook.com` |
| `FACTURA_EMISOR_COLEGIADA` | `M-40804` |
| `FACTURA_EMISOR_TELEFONO` | *(omitir secret o vacío — nulo)* |
| `FACTURA_EMISOR_IBAN` | *(omitir secret o vacío — nulo)* |
| `FACTURA_EMISOR_REGCESS` | *(omitir secret o vacío — nulo)* |
| `FACTURA_IVA_EXENCION_TEXTO` | *(opcional; el EF trae default si se omite)* |

Checklist:

- [x] `FACTURA_EMISOR_NOMBRE`
- [x] `FACTURA_EMISOR_NIF`
- [x] `FACTURA_EMISOR_DIRECCION`
- [x] `FACTURA_EMISOR_CP_CIUDAD`
- [x] `FACTURA_EMISOR_EMAIL`
- [x] `FACTURA_EMISOR_COLEGIADA`
- [x] Opcionales vacíos: `FACTURA_EMISOR_TELEFONO`, `FACTURA_EMISOR_IBAN`, `FACTURA_EMISOR_REGCESS`
...
- [x] `estado-y-pendientes.md` (este documento) — reflejado lo ejecutado en §5.1 y §6.
